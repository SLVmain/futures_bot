...
- выбор дальней границы диапазона SL: нижней для LONG и верхней для SHORT;
- срабатывание SL по `MARK_PRICE` с последующим рыночным закрытием;
- рыночный вход, если цена находится в диапазоне сигнала;
- лимитный вход по ближайшей к рынку границе, если цена вне диапазона;
...
проверяет цену Bitunix и при достижении ближайшей границы диапазона отправляет
пакет LIMIT-входов, каждый сразу со своим TP и SL. Для LONG цена LIMIT ставится
выше текущей, для SHORT — ниже. Смещение задаётся в тиках, после чего объём
заново рассчитывается от фактической LIMIT-цены и выбранного риска. Временный сбой
цены проверяется три
раза без уведомления после первой ошибки. Если цена ушла от триггера больше чем
на `0.15%` и уже находится за пределами всего диапазона входа, вход блокируется.
Пока цена остаётся внутри диапазона сигнала, бот самостоятельно продолжает
вход, даже если расстояние от ближайшей границы больше `0.15%`.
Неисполненный остаток созданного ботом LIMIT отменяется через `30` секунд;
другие ордера по символу отмена не затрагивает.
...
Когда истекает `TRIGGER_MAX_AGE_MINUTES`, триггер не удаляется и не входит в
сделку: бот спрашивает кнопками, сохранить ожидание или отменить его. Перед
сохранением текущая цена проверяется заново. Если уровень уже пройден, поздний
вход блокируется; можно только пересчитать вход по текущей цене или отменить
триггер. Команда `/triggers` показывает все локальные триггеры, которые ещё
ожидают цены, повторной активации или решения после истечения срока.
...
|---|---|---|
| `Все 3 TP` | TP1, TP2 и TP3 сигнала | `25% / 50% / 25%` |
...
| `Первые 3 TP` | только TP1, TP2 и TP3 сигнала | `25% / 50% / 25%` |
| `Только TP1` | только TP1 сигнала | `100%` на TP1 |
...
TP весь объём заново делится как `25/50/25`; доли `40/25/15` от стратегии с
...
Telegram-уведомления объединяются по группе `clientId` одной торговой идеи.
Три частичных SL и следующие за ними события закрытия дают один итог с общим
объёмом, суммарным PnL, комиссиями и funding, хотя в CSV каждая часть остаётся
отдельным событием. Контроль нескольких ожидающих limit-входов и одновременные
предупреждения об отсутствии защиты также приходят одним сообщением на идею.
Чтобы собрать связанные события Bitunix, итог закрытия может задержаться до
четырёх секунд.
...
| `/triggers` | Показывает локальные триггеры в ожидании цены и кнопки управления |
...
Исходящие автоматические уведомления Telegram записываются в системный журнал
один раз на сообщение. Chat ID, Telegram-токен и ключи Bitunix не выводятся.
